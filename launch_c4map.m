% FULL 4-CHAMBER MAP FIT (maperr only). Smoke test bh4c32a45 confirmed the
...
opts = struct();
opts.skipabr   = 1;          % m=4 abr_metric is ~38 min EACH end; pure MAP fit
...
opts.maxfe     = 300;
opts.out       = 'parfit26_c4map.mat';

t0=tic; R = parfit26(4, opts); w=toc(t0);
fprintf('\n=== C4 MAP FIT RESULT (%.0f s for %d evals) ===\n', w, opts.maxfe);
if (isstruct(R))
    fn=fieldnames(R);
