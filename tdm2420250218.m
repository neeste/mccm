...
if (iscell(stcfg))
    ...
    if(strfind(fn,'500')) fr=0.5;
    elseif(strfind(fn,'1000')) fr=1;
    elseif(strfind(fn,'2000')) fr=2;
    elseif(strfind(fn,'4000')) fr=4; 
    end
...
elseif (~ischar(stcfg)) 
    return;                                             % char ?
elseif (strfind(stcfg,'.wav')) 
    return;                                             % WAV file  ?
elseif (strfind(stcfg,'tbabr'))  
    ts=tbabr_protocol;                                  % TBABR 
elseif (strfind(stcfg,'fwdmsk')) 
    ts=fwdmsk_protocol(pr);                             % FWDMSK
elseif (strfind(stcfg,'ecog')) 
    ts=ecog_protocol(pr);                               % ECOG
elseif (strfind(stcfg,'ldngrw')) 
    ts=ldngrw_protocol(pr,st,rc);                       % LDNGRW 
elseif (strfind(stcfg,'dpoae')) 
    ts=dpoae_protocol(pr);                              % DPOAE 
elseif (strfind(stcfg,'exproto')) 
...
trt=40;             % total response time (ms)
nc=length(pr.lv);  % number of conditions
if (~isfield(pr,'ihcxx')), pr.ihcxx=zeros(nc,2); end
if (~isfield(pr,'ohcxx')), pr.ohcxx=zeros(nc,2); end
col='rgbk';
perform_calibration % calibrate, if necessary 
tic;                % start stopwatch
for k=1:nc
    mlv=pr.lv(k);         % masker level (dB SPL)
    xdr.ihcxx=pr.ihcxx(k,:);
    xdr.ohcxx=pr.ohcxx(k,:);
    wnr0=ecog_condition(mlv,xdr,0);
    wnr1=ecog_condition(mlv,xdr,1);
    if (k==1)        
        nt=length(wnr0);
        wnr=zeros(nt,nc,2);
    end
...
% plot results
lab='';
if (xdr.ihcxx(2)>xdr.ihcxx(1)), lab=[lab 'IHC ']; end
if (xdr.ohcxx(2)>xdr.ohcxx(1)), lab=[lab 'OHC ']; end
r11=wnr(:,1,1);
r12=wnr(:,1,2);
r21=wnr(:,2,1);
r22=wnr(:,2,2);
...
S.mlv=mlv;
% save wnr data
wnr=[r11 r12 r21 r22 r1d r2d];
write_data('ecog.txt',[t wnr]);
end % return
...
function wnr=ecog_condition(mlv,xdr,inv)
ts.done = 0;
mfr=0.5;     % masker frequency (kHz)
mt1=20;      % masker duration (msec)
mt2=0;       % masker start    (msec)
if (inv), mt1=-mt1; end % invert stimulus
stcfg=[mfr;mlv;mt1;mt2];
if (length(xdr.ohcxx)>1)
    ts.ohcxx=xdr.ohcxx;
end
[pa,sav,cur,ts]=tdm_init(stcfg,1,4,2,ts); % initialize model
pa.ihceq=4;     % 4=Neely synapse model
pa.hbnl=1;      % 1=nonlinear OHC transduction
if (length(xdr.ihcxx)>1)
    if (xdr.ihcxx(2)>xdr.ihcxx(1))
        nn=1+round((pa.n-1)*(xdr.ihcxx/(10*pa.xl)));
...
if (nargin>5)      % initialize
    ...
    clf
    subplot(2,1,1)
    if (ts.dsp2>1) smn=-smx/20; else smn=-smx; end
    axis([0 tt [smn smx]*1.01]);
    title(ts.lab)
    xlabel('time (ms)')
    ts.stim=pa.stim;
    switch(ts.dsp1)
