function S = tdm25(stcfg,nch,dsp1,dsp2) % gem
...
if (m), qq=xpnd_q(qst,ss,m,n); pp = cp.aa \ qq;
else, pp = qst; end
...
if (m<3)
    ...
    % 3-Chamber Logic (Updated to fix NaNs)
    for k = 1:m
        diag_idx = k + (k-1)*m;
        a1(kk, diag_idx) = -aflom(kk-1) ./ cp.abmom(kk);
        a3(kk, diag_idx) = -aflom(kk)   ./ cp.abmom(kk);
        a2(kk, diag_idx) = 1 - a1(kk, diag_idx) - a3(kk, diag_idx);
    end

    % Basal BC (Fix for singularity)
    for k = 1:m
        diag_idx = k + (k-1)*m;
        a3(1, diag_idx) = -aflom(1) ./ cp.abmom(1);
        if k==1
