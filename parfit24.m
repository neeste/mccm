% parfit24.m - fit multi-chamber model parameters to AN threshold
function parfit24
nch=3; % number of chambers
%
% compute multi-chamber cochlear model
pa=modpar24(nch);
...
midear_plt(8,flst,Dh,Zc,Gf,Ze);
%pa=fitpar24(pa,flst,pk,2); % apex
%pa=fitpar24(pa,flst,pk,1); % base
%fdm_plt(flst,xx,Yb,Pd,Db,Dh,pa.hbt,pk,nch);
%pa=fitpar24(pa,flst,pk,3); % chamber size
%pa=fitpar24(pa,flst,pk,4); % fp-map taper
for k=1:200
    % fit model parameters to data
    pa=fitpar24(pa,flst,pk,0); % partition impedance
    % compute frequency-domain cochlear model
    [xx,Yb,Pd,Db,Dh,Zc]=fdmod23(pa,flst);
    % plot model results
    fdm_plt(flst,xx,Yb,Pd,Db,Dh,pa.hbt,pk,nch);
    Ze=midear_imp(pa,flst,Zc);
    midear_plt(8,flst,Dh,Zc,Gf,Ze);
...
xpk= [ 0.81  0.71  0.59  0.47  0.33  0.19  0.05];
...
function pa=fitpar24(pa,flst,pk,ot)
mxev=120;
if (ot==0)
    pv=getpar(pa);
    mxev=960;
...
elseif (ot==4)
    ...
    error('ot=%d not implemented',ot)
end
% fit parameters to reference data
e1=get_err(pa,flst,pk);
op=optimset('MaxFunEvals',mxev);
pv=fminsearch(@(pv) refdev(pv,pa,flst,pk,ot),pv,op);
if (ot==0)
    pa=setpar(pa,pv);
    prnpar(pa,'fdm24par.txt');
elseif (ot==1)
    pa=setparme(pa,pv);
    prnparme(pa);
elseif (ot==2)
    ...
    pa.rhe=pv(2);
    pa.mhe=pv(3);
    fprintf('pa.khe=%.4g; pa.rhe=%.4g; pa.mhe=%.5f; %% ',pv)
elseif (ot==3)
    pa.chsz(1:2)=pv;
    pa.chsz=pa.chsz*(2/sum(pa.chsz));
    fprintf('pa.chsz=[%6.4f %6.4f %6.4f]; %% ',pa.chsz)
elseif (ot==4)
    pa.xtap=pv;
    fprintf('pa.xtap=%6.4f; %% ',pa.xtap)
end
e2=get_err(pa,flst,pk);
fprintf('err=%.4g %.4g\n',e1,e2)
end
...
k=0;
k=k+1;pa.k1o=exp(roundd(log(pv(k)),6));
k=k+1;pa.r1o=exp(roundd(log(pv(k)),6));
k=k+1;pa.m1o=exp(roundd(log(pv(k)),6));
k=k+1;pa.k2o=exp(roundd(log(pv(k)),6));
k=k+1;pa.r2o=exp(roundd(log(pv(k)),6));
k=k+1;pa.m2o=exp(roundd(log(pv(k)),6));
k=k+1;pa.k3o=exp(roundd(log(pv(k)),6));
k=k+1;pa.r3o=exp(roundd(log(pv(k)),6));
k=k+1;pa.k4o=exp(roundd(log(pv(k)),6));
k=k+1;pa.gpo=exp(roundd(log(pv(k)),6));
k=k+1;pa.aco=exp(roundd(log(pv(k)),6));
k=k+1;pa.bwo=exp(roundd(log(pv(k)),6));
k=k+1;pa.k1e=roundd(pv(k),4);
k=k+1;pa.r1e=roundd(pv(k),4);
k=k+1;pa.m1e=roundd(pv(k),4);
k=k+1;pa.k2e=roundd(pv(k),4);
k=k+1;pa.r2e=roundd(pv(k),4);
k=k+1;pa.m2e=roundd(pv(k),4);
k=k+1;pa.k3e=roundd(pv(k),4);
k=k+1;pa.r3e=roundd(pv(k),4);
k=k+1;pa.k4e=roundd(pv(k),4);
k=k+1;pa.gpe=roundd(pv(k),4);
k=k+1;pa.ace=roundd(pv(k),4);
k=k+1;pa.bwe=roundd(pv(k),4);
k=k+1;pa.k1q=roundd(pv(k),6);
k=k+1;pa.r1q=roundd(pv(k),6);
k=k+1;pa.m1q=roundd(pv(k),6);
k=k+1;pa.k2q=roundd(pv(k),6);
k=k+1;pa.r2q=roundd(pv(k),6);
k=k+1;pa.m2q=roundd(pv(k),6);
k=k+1;pa.k3q=roundd(pv(k),6);
k=k+1;pa.r3q=roundd(pv(k),6);
k=k+1;pa.k4q=roundd(pv(k),6);
k=k+1;pa.gpq=roundd(pv(k),6);
k=k+1;pa.acq=roundd(pv(k),6);
k=k+1;pa.bwq=roundd(pv(k),6);
end
...
function pa=setparme(pa,pv)
k=0;
k=k+1;pa.kme=exp(roundd(log(pv(k)),6));
k=k+1;pa.rme=exp(roundd(log(pv(k)),6));
k=k+1;pa.mme=exp(roundd(log(pv(k)),6));
%k=k+1;pa.ked=exp(roundd(log(pv(k)),6));
%k=k+1;pa.red=exp(roundd(log(pv(k)),6));
%k=k+1;pa.med=exp(roundd(log(pv(k)),6));
k=k+1;pa.kst=exp(roundd(log(pv(k)),6));
k=k+1;pa.rst=exp(roundd(log(pv(k)),6));
k=k+1;pa.mst=exp(roundd(log(pv(k)),6));
k=k+1;pa.kim=exp(roundd(log(pv(k)),6));
k=k+1;pa.rim=exp(roundd(log(pv(k)),6));
end

function prnparme(pa)
fprintf('pa.kma=%.6g;  ',pa.kma);
fprintf('pa.rma=%.6g;  ',pa.rma);
fprintf('pa.mma=%.6g;\n',pa.mma);
fprintf('pa.ked=%.6g;  ',pa.ked);
fprintf('pa.red=%.6g;  ',pa.red);
fprintf('pa.med=%.6g;\n',pa.med);
fprintf('pa.kst=%.6g;  ',pa.kst);
fprintf('pa.rst=%.6g;  ',pa.rst);
fprintf('pa.mst=%.6g;\n',pa.mst);
fprintf('pa.kim=%.6g;  ',pa.kim);
fprintf('pa.rim=%.6g; %% ',pa.rim);
end
