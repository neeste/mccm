% tdm25 - multi-chamber time-domain cochlear model.
%   S = tdm25(STCFG,NCH,DSP1,DSP2) computes a cochlear-model response
...
function S = tdm25(stcfg,nch,dsp1,dsp2)
...
cur.hc = zeros(pa.n,1);      % HC current
...
while (cur.cyc < pa.ncyc)               % time-step loop ---------------
    ...
    cur = hc_step(pa,cur,nf);           % update HC & HB state
...
report_level(sav,ts);           % compute SPL at eardrum
plot_ts(pa,sav,cur,ts);         % time-step plot (summary)
[mbf,bf]=plot_sv(pa,sav,ts,cp); % saved-variables plot
end % return
...
n = pa.n;
ii=1:n;
dd = cur.d(ii) - cur.d(ii+n); % HB deflection
% OHC receptor current
met=pa.met;
hco=pa.hco;
hcs=pa.hcs;
b0=1./(1+exp(hco/hcs));
b1=1./(1+exp((hco-dd)/hcs));
cur.hc=(b1-b0)*hcs*met;
% IHC-HB deflection
if (pa.ihcv==0)  % HB displacement or velocity ?
    cur.hb = dd;      % displacement
else
    v1 = cur.v(ii);
    v2 = cur.v(ii+n);
    cur.hb = v1 - v2; % velocity
end
cur.nr = cur.hb.*pa.synpro; % neural rate without synapse
...
ts.stcfg = stcfg;             % stimulus configuration
...
ts.dsp2 = dsp2;   % 1=HB 2=NR 3=LD 4=HC
...
elseif (stcfg(1,1)>0)     % toneburst stimulus
...
stcfg=[fr;lv;bw;st;rc]; % ldngrw stimulus configuration
pa=modpar24(1);
pa.ntsf=4096;           % increase stimulus duration
pa.nstp=pa.ntsf*pa.ntsw;
[pa,sav,cur,ts]=tdm_init(stcfg,1,0,0,ts,pa); % initialize model
pa.ihceq=4;             % Neely synapse model
pa.hbnl=1;              % nonlinear OHC transduction
if ((nargin<6)||(ew==0))
    ew=pa.ldew;
...
elseif (nargin==5) % time-step plot
    ...
    jj=pa.ldne:-1:1;
    switch(ts.dsp2)
    case 1, yy=sav.hb(ii,kk)'; % HB
    case 2, yy=sav.nr(ii,kk)'; % NR
    case 3, yy=sav.ld(ii,jj)'; % LD
    case 4, yy=sav.hc(ii,kk)'; % HC
    end
    xc=[0 ts.dt*ts.ntsf*pa.ntsw];
    yc=[pa.xl*10 0];
    imagesc(xc,yc,yy)
    xlabel('time (ms)')
    if (pa.m)
    ...
    if (pa.m)
        ...
        case 4, title('hair-cell current');           % HC
        end
    end
    if (ts.dsp2==3)
...
else, [mbf,bf]=plot_click(pa,sav,ts,cp,fig); end % plot click response
end % return
...
plot_cal(pa,sav,fig)
[mbf,bf]=plot_tc(pa,sav,ts,cp,pgd,fig+1);
end % return
...
emZe=10^4*[22.8 13.3 6.5 4.8 4.1 4.0 4.9];      % (mks ohm)
emZc=1e6*[8.36 6.28 9.936 10.7 15.7 22.6 19.8]; % (mks ohm)
...
zc=zc*10^5;
zm=zm*10^5;
% plot Pe/Ve and Pe/Ps
pc_db=dbmag(pc_gn);
ps_db=dbmag(ps_gn);
zc_mg=max(abs(zc),1e-9)/1000;
zm_mg=max(abs(zm),1e-9)/1000;
zm_ph=angle(zm)/(2*pi);
ps_ph=angle(pc_gn)/(2*pi);
figure(fig);clf
subplot(2,3,1)
loglog(f,zm_mg,ef,emZe,'r')
xlim([0.1 20])
ylabel('Zm (mks k\Omega)')
title('middle-ear impedance')
subplot(2,3,4)
semilogx(f,zm_ph)
ylabel('phase (cyc)')
axis([0.1 20 [-1 1]*0.301])
subplot(2,3,3)
loglog(f,zc_mg,ef,emZc,'r')
xlim([0.1 20])
ylabel('Zc (mks k\Omega)')
title('cochlea impedance')
subplot(2,3,2)
semilogx(f,ps_db,ef,emGf,'r')
axis([0.1 20 -40 40])
xlabel('frequency (kHz)')
ylabel('Ps / Pe (dB)')
...
for i=1:nsv
    ...
    if (pa.ihceq<1)
        ...
        sv = pa.isv(i);
        vh = (cp.hb(sv,1).*d1+cp.hb(sv,2).*d2).*s; % HB velocity (cm/s)
    end
    mnvh=1e-22;
    vh(isnan(vh)|(abs(vh)<mnvh)) = mnvh;
    pe = pe / spl_ref;
...
pa.gam = 1;                    % CA gain
pa.gampro = ones(pa.n,1);      % CA-gain profile
pa.synpro = ones(pa.n,1);      % synaptopathy profile
...
pa.ihceq=4; % 4 = Neely synapse model
pa.hbnl=0; pa.hbsc=0.04; pa.hbmx=6e-9;
pa.ihctc=0.2e-3; pa.ihcsf=1e5; pa.nrgn=2.5e6; pa.ew=0.1;
pa.ihcex=14; pa.ihcrr=17.836; pa.ihcdr=1.2637; pa.ihcv=1;
