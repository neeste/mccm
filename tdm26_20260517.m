...
%
%   USAGE: S = tdm25(STCFG,NCH,DSP1,DSP2)
%
...
%
%   Improvements by GEMINI [Nov-2025]:
%   - Vectorized column indexing in 'xpnd_a'
%   - 3-chamber fluid dynamics
...
if (nargin<6), pa=modpar24(nch); end
...
if (m<3)
    ...
    % 3-Chamber Logic (Increment 2: Safe Mass Scaling)
    % Define a NaN-safe mass ratio tracking the baseline parameters
    mu = cp.m1 ./ max(cp.m2, 1e-12);
    
    for k = 1:m
        diag_idx = k + (k-1)*m;
        
        % Chamber 1 (SV) and Chamber 3 (ST) use standard spatial scaling
        if k == 1 || k == 3
            a1(kk, diag_idx) = -(aflom(kk-1) * pa.chsz(k)) ./ cp.abmom(kk);
            a3(kk, diag_idx) = -(aflom(kk)   * pa.chsz(k)) ./ cp.abmom(kk);
        % Chamber 2 (SS) scales explicitly by the mass ratio mu
        elseif k == 2
            a1(kk, diag_idx) = -(aflom(kk-1) * pa.chsz(k)) .* mu(kk-1) ./ cp.abmom(kk);
            a3(kk, diag_idx) = -(aflom(kk)   * pa.chsz(k)) .* mu(kk)   ./ cp.abmom(kk);
        end
        
        % Keep the hardcoded 1 that preserves integration stability
        a2(kk, diag_idx) = 1 - a1(kk, diag_idx) - a3(kk, diag_idx);
    end
    ...
    for k = 1:m
        diag_idx = k + (k-1)*m;
        if k == 2
            a3(1, diag_idx) = -(aflom(1) * pa.chsz(k)) * mu(1) ./ cp.abmom(1);
        else
            a3(1, diag_idx) = -(aflom(1) * pa.chsz(k)) ./ cp.abmom(1);
        end
        
        if k==1
            a2(1, diag_idx) = 1 + cp.alfx - a3(1, diag_idx); 
        elseif k==m
            a3(1, diag_idx) = 0;
            a2(1, diag_idx) = 1; % Open Round Window Clamp
        else
            a2(1, diag_idx) = 1 - a3(1, diag_idx); 
        end
    end
...
for j=1:m
    ...
    % The logic in original tdm25 assumes implicit alignment.
    % aaa(ii) maps to specific columns of [a1 a2 a3].
    % In vectorized form, we map ii_local of ad to the columns of AAA_j.
    % Actually, the loop structure in scalar code maps:
    % ad(kk, :) = [a1(k,jj) a2(k,jj) a3(k,jj)](ii)
    % This means ad rows get reordered values from AAA.

    % FIX: Use the scalar loop approach logic for selection but vectorized
    % We need to map the columns of AAA_j (1:3m) to the columns of ad (1:2m+1).
    % The mapping is defined by 'ii' in the original scalar loop: ii=(1:nd)+(j-1).
    % BUT wait, 'ii' is the DESTINATION index in the original scalar code?
    % No: "ad(kk,:) = aaa(ii)". 'ii' is the SOURCE index in 'aaa'.
...
elseif (m==3), j3 = (1:n) * 3; j2 = j3 - 1; j1 = j2 - 1; qx(j1) = s1 + qst;
end
end
...
elseif (m==2)
    j2=(1:n)*2; j1=j2-1; s1 = s1 - (pp(j1) - pp(j2)) / 2;
    a(i1) = s1 ./ cp.m1; a(i2) = s2 ./ cp.m2;
elseif (m==3)
    j3=(1:n)*3; j2=j3-1; j1=j2-1;
    s1 = s1-(pp(j1) - pp(j3)); s2 = s2-(pp(j2) - pp(j3));
    a(i1) = s1 ./ cp.m1; a(i2) = s1 ./ cp.m1 + s2 ./ cp.m2;
end
a(i1(1)) = 0; a(i1(n)) = 0;
end
...
elseif (pa.m==3)
    % Corrected algebra: k2 is relative shear, k3 is absolute anchor
    k_act = cp.gh .* cp.k3 - gam .* cp.k4;
    r_act = cp.gh .* cp.r3;

    % BM Force 
    s1_pass = cp.k1 .* d1 + cp.r1 .* v1;
    s1_act  = k_act .* d2 + r_act .* v2;
    s1 = -(s1_pass + s1_act);

    % HB Force 
    s2_rel = cp.k2 .* (d2 - d1) + cp.r2 .* (v2 - v1);
    s2_abs = cp.k3 .* d2 + cp.r3 .* v2;
    s2 = -(s2_rel + s2_abs);
end
...
f=sav.f; s=2i*pi*f; nf=length(f); spl=zeros(nf,nsv); phv=zeros(nf,nsv); gdv=zeros(nf,nsv); qe=zeros(1,nsv); pbf=zeros(1,nsv);
dsp_ref = 140; spl_ref = 0.0002; j1=ceil(nf*(0.1/max(f)));
for i=1:nsv
    [d1,d2,pe,~,~,~,~,nr] = fetch_sav(i,sav);
    if (pa.ihceq<1), vh = nr/1000; else, sv = pa.isv(i); vh = (cp.hb(sv,1).*d1+cp.hb(sv,2).*d2).*s; end
...
figure(fig);clf; subplot(2,1,1); reset_color_index; semilogx(f,spl,fpk/1000,mpk,'ro',bf,mbf,'k.'); axis([0.1 20 0 100]); ylabel('Pe (dB SPL)');
...
if (pr==0)
    ...
    legend('tdm25','Fletcher','Location','SouthEast')
    ...
    axis([-5 105 -5 105])
    legend('tdm25','Rasetshwane','Location','NorthWest')
    drawnow
    %----
...
if (nch<3), pa=par_CEL25; else, pa=modpar23c3; end
pa.m=nch; pa.xp=1; pa.nstp = 20480; pa.ncyc = 1; pa.ntsw = 10; pa.ntsf = 2048; pa.dt = 2e-6; pa.nimp = 1;
pa.ME_VEP_SCALE = 10; pa.ME_VST_SCALE_PHANTOM = 2; pa.IHC_SUBSTEPS = 10;
end

function pa=par_CEL25
pa=par_CEL16; pa.rma=pa.rma/2; pa.rst=pa.rst/2; pa.kma=pa.kma*4; pa.kst=pa.kst*4; pa.red=pa.red*5;
end

function pa=par_CEL16
pa.parlab='CEL16'; pa.chsz=[1 1]; pa.m = 1; pa.n = 1401; pa.gam = 1; pa.gampro = ones(pa.n,1); pa.synpro = ones(pa.n,1); pa.xl = 3.5; pa.yw = 0.1; pa.zh = 0.1; pa.rho = 1; pa.stgain=0.0127;
pa.isv=[1136 1005 840 655 466 273 80]; pa.hbt=8.5; pa.xtap=0.2339; pa.xtex=6; pa.mmeq=1; pa.khe=0.0001; pa.rhe=0.0001; pa.mhe=0.01;
pa.nmev=4; pa.kme=0.1; pa.rme=400; pa.mme=0.1; pa.acp=0.5; pa.adi=0.2; pa.aed=0.5; pa.ama=0.5; pa.ast=0.03; pa.cep=2700;  pa.rep=29.16; pa.mevgn=1.262; pa.stim=3;
pa.mcp=0.0002; pa.rcp=0.1; pa.kcp=2200; pa.rfz=0.01; pa.mdi=0.005;  pa.rdi=100;  pa.kdi=7.7e+06; pa.mrw=5e-3; pa.rrw=20; pa.krw=1.5e5; pa.arw=0.0625; pa.mco=3; pa.rco=1e5;  pa.gme=0.5;
pa.kma=3e+06;  pa.rma=8000;  pa.mma=0.85; pa.ked=7.04632e+06;  pa.red=200;  pa.med=0.02; pa.kst=4.66022e+06;  pa.rst=8140.1;  pa.mst=0.89833; pa.kim=3e+08;  pa.rim=28508.7;
pa.ihceq=4; pa.hbnl=0; pa.hbsc=0.04; pa.hbmx=6e-9; pa.ihctc=0.2e-3; pa.ihcsf=1e5; pa.nrgn=2.5e6; pa.ew=0.1; pa.ihcex=14; pa.ihcrr=17.836; pa.ihcdr=1.2637; pa.ihcv=1;
pa.ldew=2; pa.ldne=70; pa.ldsc=[6.913 3.823 0.454 4.499]; pa.met=200; pa.hco=1e-4; pa.hcs=1e-4;
pa.k1o=  2.394e+08; pa.k1e=-2.6655; pa.k1q= 0.000000; pa.r1o=      871.4; pa.r1e=-1.3937; pa.r1q= 0.000000; pa.m1o=   0.007417; pa.m1e=-0.1884; pa.m1q= 0.000000;
pa.k2o=  3.036e+08; pa.k2e=-3.4762; pa.k2q= 0.000000; pa.r2o=       1979; pa.r2e=-1.2466; pa.r2q= 0.000000; pa.m2o=    0.03417; pa.m2e=-0.0828; pa.m2q= 0.000000;
pa.k3o=  3.151e+08; pa.k3e=-2.9092; pa.k3q= 0.000000; pa.r3o=      1.049; pa.r3e= 0.1033; pa.r3q= 0.000000; pa.k4o=  4.045e+08; pa.k4e=-2.7948; pa.k4q= 0.000000;
pa.r4o=          0; pa.r4e= 0.0000; pa.r4q= 0.000000; pa.gpo=          1; pa.gpe= 0.0000; pa.gpq= 0.000000; pa.aco=       0.01; pa.ace=-0.4000; pa.acq= 0.000000;
pa.bwo=       0.05; pa.bwe= 0.0000; pa.bwq= 0.000000;
end

function pa=modpar23c3
pa.parlab='cel24c3'; pa.chsz=[0.9 0.1 1.0]; pa.m = 1; pa.n = 1401; pa.gam = 1; pa.gampro = ones(pa.n,1); pa.synpro = ones(pa.n,1); pa.xl = 3.5; pa.yw = 0.1; pa.zh = 0.1; pa.rho = 1; pa.stgain=0.0127;
pa.isv=[1136 1005 840 655 466 273 80]; pa.hbt=33; pa.xtap=0.2339; pa.xtex=6; pa.mmeq=1; pa.khe=0.0001; pa.rhe=0.0001; pa.mhe=0.01;
pa.nmev=4; pa.kme=0.1; pa.rme=400; pa.mme=0.1; pa.acp=0.5; pa.adi=0.2; pa.aed=0.5; pa.ama=0.5; pa.ast=0.03; pa.cep=2700;  pa.rep=29.16; pa.mevgn=1.262; pa.stim=3;
pa.mcp=0.0002; pa.rcp=0.1; pa.kcp=2200; pa.rfz=0.01; pa.mdi=0.005;  pa.rdi=100;  pa.kdi=7.7e+06; pa.mrw=5e-3; pa.rrw=20; pa.krw=1.5e5; pa.arw=0.0625; pa.mco=3; pa.rco=1e5;  pa.gme=0.5;
pa.kma=3e+06;  pa.rma=8000;  pa.mma=0.85; pa.ked=7.04632e+06;  pa.red=200;  pa.med=0.02; pa.kst=4.66022e+06;  pa.rst=8140.1;  pa.mst=0.89833; pa.kim=3e+08;  pa.rim=28508.7;
pa.ihceq=4; pa.hbnl=0; pa.hbsc=0.04; pa.hbmx=6e-9; pa.ihctc=0.2e-3; pa.ihcsf=1e5; pa.nrgn=2.5e6; pa.ew=0.1; pa.ihcex=14; pa.ihcrr=17.836; pa.ihcdr=1.2637; pa.ihcv=1;
pa.ldew=2; pa.ldne=70; pa.ldsc=[6.913 3.823 0.454 4.499]; pa.met=200; pa.hco=1e-4; pa.hcs=1e-4;
pa.k1o=2.41443e+08; pa.k1e=-2.7377; pa.k1q= 0.000002; pa.r1o=    1056.85; pa.r1e=-1.3138; pa.r1q= 0.000002; pa.m1o=  0.0074497; pa.m1e=-0.2012; pa.m1q= 0.000002;
pa.k2o=3.05082e+08; pa.k2e=-3.5063; pa.k2q= 0.000002; pa.r2o=    1984.21; pa.r2e=-1.2479; pa.r2q= 0.000002; pa.m2o=  0.0360281; pa.m2e=-0.0812; pa.m2q= 0.000002;
pa.k3o=3.15079e+08; pa.k3e=-2.9231; pa.k3q= 0.000003; pa.r3o=    1.10439; pa.r3e= 0.1065; pa.r3q= 0.000002; pa.k4o=4.04629e+08; pa.k4e=-2.8017; pa.k4q= 0.000001;
pa.r4o=          0; pa.r4e= 0.0000; pa.r4q= 0.000000; pa.gpo=   0.995591; pa.gpe= 0.0000; pa.gpq= 0.000003; pa.aco= 0.00993884; pa.ace=-0.4324; pa.acq= 0.000002;
