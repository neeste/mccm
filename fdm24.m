% fdm24 - multi-chamber frequency-domain model of cochlea
function fdm24(nch)
if (nargin<1), nch=0; end % number of cochlear fluid chambers
pa=modpar24(nch);        % fetch model parameters
...
% plot tuning curves
flst2=500*2.^linspace(-2,5.3,100);  % frequency list 2
pa.gam=1; % active
[~,~,~,~,Dh,Zc,Gf]=fdmod23(pa,flst2);
Ze=midear_imp(pa,flst2,Zc);
midear_plt(8,flst2,Dh,Zc,Gf,Ze);
